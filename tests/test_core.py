from swift_files.core import build_manifest, find_duplicates, hash_file, inspect_file, quarantine_duplicates, verify_manifest, write_manifest
...
def test_manifest_round_trip_and_drift(tmp_path):
    ...
    manifest = tmp_path / "manifest.json"; write_manifest(root, manifest)
...
def test_manifest_detects_missing_and_strict_unexpected(tmp_path):
    root = tmp_path / "root"; root.mkdir(); (root / "a.txt").write_text("one", encoding="utf-8")
    manifest = tmp_path / "manifest.json"; write_manifest(root, manifest)
    (root / "a.txt").unlink(); (root / "b.txt").write_text("new", encoding="utf-8")
